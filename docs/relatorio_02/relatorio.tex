...
arquivo \texttt{toy.mp4} (166 quadros).}
...
fundo (branco). Para cada método são apresentados o histograma da imagem original (com a
linha do limiar) e a fração de pixels pretos.
...
O vídeo utilizado foi \texttt{toy.mp4}, com $640\times480$ pixels e 166 quadros.
...
qualquer deslocamento do cenário entre frames aumenta artificialmente a contagem de pixels
alterados, podendo gerar falsos positivos. Para \texttt{toy.mp4} (vídeo com movimentos
suaves), nenhuma transição foi detectada com $T_2 = 5\%$, indicando que o vídeo não
contém cortes abruptos ou que os parâmetros precisam ser ajustados para o conteúdo
específico.}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{\figdir{secao_02/metodo_10_pixels}/pixels_transicoes.png}
  \caption{Métrica por quadro — Diferenças entre Pixels (\texttt{toy.mp4})}
\end{figure}
...
mais sensíveis a ruído, maiores são menos sensíveis a mudanças locais. Para
\texttt{toy.mp4}, assim como no método de pixels, nenhuma transição foi detectada.}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{\figdir{secao_02/metodo_11_blocos}/blocos_transicoes.png}
  \caption{Métrica por quadro — Diferenças entre Blocos (\texttt{toy.mp4})}
\end{figure}
...
tons), porém insensível a movimentos internos a uma cena. Com $\alpha = 4$, o limiar
adaptativo $T = \mu + 4\sigma$ é bastante conservador, exigindo picos de $D_i$ muito
acima da média para ser declarado como corte. O gráfico resultante para \texttt{toy.mp4}
mostra variações suaves na série $D_i$, confirmando a ausência de transições abruptas.}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{\figdir{secao_02/metodo_12_histogramas}/histogramas_transicoes.png}
  \caption{Métrica por quadro — Diferenças entre Histogramas (\texttt{toy.mp4})}
\end{figure}
...
Para os três detectores de transições em vídeo, nenhuma transição foi detectada no vídeo
\texttt{toy.mp4} com os parâmetros padrão, o que é coerente com o conteúdo do vídeo (sem
cortes abruptos visíveis). O método baseado em pixels apresentou o maior ganho de
vetorização (209$\times$), enquanto o de histogramas mostrou ganho moderado (11,6$\times$)
por ter menor número de iterações por frame. Os três métodos convergem na ausência de
transições para este vídeo, o que sugere consistência entre as abordagens.
