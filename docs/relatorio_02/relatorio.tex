...
O vídeo utilizado foi \texttt{umn.mp4}, disponível no servidor do professor.
...
transição abrupta. Parâmetros utilizados: $T_1 = 25$, $T_2 = 0{,}05 \times W \times H$
...
alterados, podendo gerar falsos positivos. Para minimizar isso, $T_1 = 25$ filtra
variações de ruído menores que 25 níveis de cinza; $T_2 = 5\%$ garante que apenas
quadros onde realmente a maioria da cena muda sejam marcados como corte. Os picos no
gráfico correspondem aos cortes abruptos detectados no vídeo \texttt{umn.mp4}.}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{\figdir{secao_02/metodo_10_pixels}/pixels_transicoes.png}
  \caption{Métrica por quadro — Diferenças entre Pixels (\texttt{umn.mp4})}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Métrica por quadro — Diferenças entre Blocos (\texttt{umn.mp4})}
\end{figure}
...
$\alpha = 3$ (limite inferior do intervalo sugerido pelo enunciado, conferindo maior
sensibilidade a cortes de baixo contraste).}
...
tons), porém insensível a movimentos internos a uma cena. Com $\alpha = 3$, o limiar
adaptativo $T = \mu + 3\sigma$ equilibra sensibilidade e robustez: picos que representam
cortes reais ficam visivelmente acima do limiar, enquanto variações contínuas de
iluminação e movimento permanecem abaixo.}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{\figdir{secao_02/metodo_12_histogramas}/histogramas_transicoes.png}
  \caption{Métrica por quadro — Diferenças entre Histogramas (\texttt{umn.mp4})}
\end{figure}
...
Para os três detectores de transições em vídeo, o arquivo \texttt{umn.mp4} foi
utilizado por ser um filme mudo com cortes abruptos frequentes entre tomadas — cenário
ideal para validar os algoritmos. Os três métodos detectam os mesmos eventos de transição
com ligeiras diferenças nos quadros exatos, demonstrando consistência entre as abordagens.
O método baseado em pixels ($T_1=25$, $T_2=5\%$) apresentou o maior ganho de vetorização
(209$\times$); o de histogramas ($B=256$, $\alpha=3$) é o mais robusto a movimentos de
câmera por operar sobre a distribuição global em vez de pixels individuais.
