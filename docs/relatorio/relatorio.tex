...
\newcommand{\itemsubsection}[2]{%
  \subsection*{Item #1 -- #2}%
  \phantomsection%
  \addcontentsline{toc}{subsection}{Item #1 -- #2}%
}
...
\campo{Linguagem e bibliotecas}{A implementação foi desenvolvida em Python 3. O projeto utiliza a biblioteca Pillow apenas para leitura e escrita de imagens PNG, enquanto as transformações pedidas no enunciado são implementadas manualmente no código, sem uso de funções prontas de rotação. O restante da infraestrutura utiliza apenas a biblioteca padrão do Python.}
\campo{Entrada de dados}{As imagens de entrada estão no formato PNG, conforme especificado no trabalho. No item 1.1 foi utilizada a imagem monocromática \arquivo{baboon\_monocromatica.png}, disponibilizada no site do professor e baixada automaticamente para \arquivo{data/input/exercicio\_01/}.}
...
O projeto foi estruturado para separar a leitura e escrita das imagens das rotinas de processamento. Essa divisão facilita o reaproveitamento de código entre os itens do trabalho e torna mais clara a validação de cada transformação isoladamente.
...
\campo{Representação das imagens}{Para o exercício 1, a imagem monocromática é carregada como uma matriz bidimensional de inteiros no intervalo $[0,255]$, indexada no formato linha-coluna. Assim, cada pixel pode ser acessado por \arquivo{imagem[i][j]}, o que facilita a aplicação direta das fórmulas de remapeamento de índices.}
\campo{Convenções adotadas}{Os valores de intensidade permanecem no intervalo $[0,255]$ durante toda a execução do item 1.1, de modo que não há necessidade de clipping adicional nessa etapa. Nas rotações de $90^\circ$ e $270^\circ$, as dimensões da imagem de saída são trocadas para preservar corretamente a geometria do resultado.}
...
\campo{Esboço do trabalho}{A implementação foi organizada em quatro blocos principais: (i) preparação automática das entradas, incluindo download de arquivos quando necessário, (ii) processamento específico de cada exercício, (iii) gravação dos resultados em pastas próprias, com cópia das figuras relevantes para o relatório em LaTeX, e (iv) compilação opcional do relatório por um script separado. Dessa forma, um problema de compilação em LaTeX não interrompe a geração dos resultados dos exercícios.}
\campo{Estrutura do relatório}{Nesta versão do projeto, o item 1.1 foi implementado e documentado em detalhe. As demais subseções permanecem como estrutura-base para preenchimento à medida que os outros exercícios forem sendo desenvolvidos.}
...
\campo{Conjunto de testes}{Para o item 1.1 foi utilizada a imagem \arquivo{baboon\_monocromatica.png}. Essa imagem é apropriada para o teste porque possui textura rica e orientação visual bem definida, o que facilita verificar se a rotação foi aplicada corretamente.}
\campo{Registro das saídas}{As saídas do item 1.1 são salvas em \arquivo{results/exercicio\_01/} com nomes descritivos para cada ângulo: \arquivo{baboon\_rotacao\_90\_horario.png}, \arquivo{baboon\_rotacao\_180.png} e \arquivo{baboon\_rotacao\_270\_horario.png}. As imagens usadas no relatório são copiadas para \arquivo{docs/relatorio/figuras/exercicio\_01/}, enquanto a geração do PDF final é feita separadamente a partir de \arquivo{docs/relatorio/relatorio.tex}.}
\campo{Critérios de validação}{A corretude foi verificada por inspeção visual e pela conferência da fórmula de mapeamento de índices. Também foi observado se as rotações de $90^\circ$ e $270^\circ$ mantinham a imagem coerente com a troca de dimensões esperada.}

\section{Resultados e Discussão}

Esta seção apresenta os resultados obtidos em cada item do Trabalho 1. A organização abaixo segue a numeração do enunciado para manter o relatório alinhado com a especificação oficial da atividade. Na versão atual do projeto, apenas o item 1.1 está implementado de forma completa; as demais subseções foram mantidas como roteiro para continuação do desenvolvimento.

\itemsubsection{1.1}{Rotação de Imagens em Múltiplos de 90 Graus}
...
\begin{figure}[H]
  ...
  \end{minipage}
  \caption{Resultados obtidos para o item 1.1 com a imagem \arquivo{baboon\_monocromatica.png}.}
\end{figure}

\itemsubsection{1.2}{Ampliação de Imagem por Replicação}
...
\itemsubsection{1.3}{Esboço a Lápis}
...
\itemsubsection{1.4}{Ajuste de Brilho}
...
\itemsubsection{1.5}{Binarização por Limiar}
...
\itemsubsection{1.7}{Alteração de Cores}
...
\itemsubsection{1.8}{Transformação de Imagens Coloridas}
...
\itemsubsection{1.9}{Planos de Bits}
...
\itemsubsection{1.10}{Combinação de Imagens}
...
\itemsubsection{1.11}{Transformação de Intensidade}
...
\itemsubsection{1.12}{Quantização de Imagens}
...
\itemsubsection{1.13}{Filtragem de Imagens}
