...
def _save_metric_plot(
    ...
    ax.plot(range(len(metrics)), metrics, color="steelblue", linewidth=0.8, label="D_i")
    ax.axhline(y=threshold, color="orange", linestyle="--", linewidth=1.0, label=f"T = {threshold:.1f}")
...
def detect_transitions_histogramas(
    ...
    hists = [
        np.histogram(f, bins=bins, range=(0, 256), density=True)[0].astype(np.float64)
        for f in frames
    ]
...
def process(input_paths: dict[str, Path], output_dir: Path) -> list[Path]:
    ...
    metrics, transitions, threshold = detect_transitions_histogramas(frames, BINS, ALPHA)
    ...
    _save_metric_plot(metrics, transitions, threshold, output_path, "Diferenças entre Histogramas")
    return [output_path]
...
def report_files() -> dict[str, Path]:
    ...
    return {
        "histogramas_transicoes.png": output_dir / "histogramas_transicoes.png",
    }
