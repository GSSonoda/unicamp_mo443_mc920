...
def threshold_bernsen_loop(
    ...
    for i in range(h):
        for j in range(w):
            ...
            # Limiar local: ponto médio entre mínimo e máximo da janela
...
def threshold_bernsen(
    ...
    img = np.array(image, dtype=np.float64)
    # Filtros de mínimo e máximo calculam zmin/zmax para cada posição
    zmin = minimum_filter(img, size=n, mode="nearest")
    zmax = maximum_filter(img, size=n, mode="nearest")
    # Limiar local: média entre mínimo e máximo da vizinhança
    t = (zmin + zmax) / 2.0
    return np.where(img > t, 0, 255).astype(np.uint8).tolist()


def _threshold_bernsen_map(
    image: list[list[int]], n: int = N
) -> np.ndarray:
...
def process(input_paths: dict[str, Path], output_dir: Path) -> list[Path]:
    ...
    for stem, img_path in input_paths.items():
        ...
        # Binariza com Bernsen e obtém a média do mapa de limiares
        binary = threshold_bernsen(image, N)
        t_map = _threshold_bernsen_map(image, N)
        threshold_val = float(t_map.mean())

        # Salva original e binarizada
        saved += save_grayscale_outputs(output_dir, {
            f"bernsen_{stem}_original.png": image,
            f"bernsen_{stem}_binaria.png": binary,
        })

        # Salva histograma com a média dos limiares locais
        hist_path = output_dir / f"bernsen_{stem}_histograma.png"
        _save_histogram(
...
def report_files() -> dict[str, Path]:
    ...
    files = {}
    for stem in stems:
        files[f"bernsen_{stem}_original.png"] = (
            output_dir / f"bernsen_{stem}_original.png"
        )
        files[f"bernsen_{stem}_binaria.png"] = (
            output_dir / f"bernsen_{stem}_binaria.png"
        )
        files[f"bernsen_{stem}_histograma.png"] = (
            output_dir / f"bernsen_{stem}_histograma.png"
        )
    return files
