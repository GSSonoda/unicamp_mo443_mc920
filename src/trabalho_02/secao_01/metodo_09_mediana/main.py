...
def threshold_mediana_loop(
    ...
    for i in range(h):
        for j in range(w):
            ...
            # Limiar local: mediana dos pixels da janela
            t = np.median(window)
            result[i, j] = 0 if img[i, j] > t else 255
    return result.tolist()


def threshold_mediana(
    image: list[list[int]], n: int = N
) -> list[list[int]]:
    """Método da mediana vetorizado com scipy median_filter."""
    img = np.array(image, dtype=np.float64)
    # median_filter calcula a mediana local em janela n×n para cada pixel
    t = median_filter(img, size=n, mode="nearest")
    return np.where(img > t, 0, 255).astype(np.uint8).tolist()


def _threshold_mediana_map(
    image: list[list[int]], n: int = N
) -> np.ndarray:
    """Retorna o mapa de limiares locais (para visualização no histograma)."""
    img = np.array(image, dtype=np.float64)
    # O mapa de limiares é exatamente a saída do median_filter
    return median_filter(img, size=n, mode="nearest")
...
def _save_histogram(
    ...
    # Linha vertical na média global dos limiares de mediana local
    ax.axvline(
...
def process(input_paths: dict[str, Path], output_dir: Path) -> list[Path]:
    ...
    for stem, img_path in input_paths.items():
        ...
        # Binariza com mediana local e obtém a média do mapa de limiares
        binary = threshold_mediana(image, N)
        t_map = _threshold_mediana_map(image, N)
        threshold_val = float(t_map.mean())

        # Salva original e binarizada
        saved += save_grayscale_outputs(output_dir, {
            f"mediana_{stem}_original.png": image,
            f"mediana_{stem}_binaria.png": binary,
        })

        # Salva histograma com a média dos limiares locais
        hist_path = output_dir / f"mediana_{stem}_histograma.png"
        _save_histogram(
...
def report_files() -> dict[str, Path]:
    ...
    files = {}
    for stem in stems:
        files[f"mediana_{stem}_original.png"] = (
            output_dir / f"mediana_{stem}_original.png"
        )
        files[f"mediana_{stem}_binaria.png"] = (
            output_dir / f"mediana_{stem}_binaria.png"
        )
        files[f"mediana_{stem}_histograma.png"] = (
            output_dir / f"mediana_{stem}_histograma.png"
        )
    return files
