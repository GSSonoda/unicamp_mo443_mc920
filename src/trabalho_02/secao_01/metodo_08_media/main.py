...
def threshold_media_loop(
    image: list[list[int]], n: int = N, c: float = C
) -> list[list[int]]:
    ...
    for i in range(h):
        for j in range(w):
            ...
            # Limiar: média local deslocada pela constante C
            t = window.mean() - c
...
def threshold_media(
    ...
    img = np.array(image, dtype=np.float64)
    # Calcula média local em janela n×n via filtro uniforme
    mu = uniform_filter(img, size=n, mode="nearest")
    # Limiar local: média deslocada pela constante C
    t = mu - c
    return np.where(img > t, 0, 255).astype(np.uint8).tolist()


def _threshold_media_map(
    image: list[list[int]], n: int = N, c: float = C
) -> np.ndarray:
...
def _save_histogram(
    ...
    # Linha vertical na média dos limiares locais (mu - C)
    ax.axvline(
...
def process(input_paths: dict[str, Path], output_dir: Path) -> list[Path]:
    ...
    for stem, img_path in input_paths.items():
        ...
        # Binariza com média local e obtém a média do mapa de limiares
        binary = threshold_media(image, N, C)
        t_map = _threshold_media_map(image, N, C)
        threshold_val = float(t_map.mean())

        # Salva original e binarizada
        saved += save_grayscale_outputs(output_dir, {
            f"media_{stem}_original.png": image,
            f"media_{stem}_binaria.png": binary,
        })

        # Salva histograma com a média dos limiares locais
        hist_path = output_dir / f"media_{stem}_histograma.png"
        _save_histogram(
...
def report_files() -> dict[str, Path]:
    ...
    files = {}
    for stem in stems:
        files[f"media_{stem}_original.png"] = (
            output_dir / f"media_{stem}_original.png"
        )
        files[f"media_{stem}_binaria.png"] = (
            output_dir / f"media_{stem}_binaria.png"
        )
        files[f"media_{stem}_histograma.png"] = (
            output_dir / f"media_{stem}_histograma.png"
        )
    return files
